...
### Added
- **WebSocket failover** ([curl_fo](https://github.com/idrto/curl_fo)-style): `FailoverWebSocket.connect` for `ws://` with DNS cache, Happy Eyeballs cold-path connect, sequential warm-path connect, and reconnect policy (retry same IP once, then next ranked IP). `FailoverDio.connectWebSocket` convenience alias. Events: `WebSocketConnected`, `WebSocketReconnecting`, `WebSocketReconnected`, `WebSocketReconnectFailed`. `wss://` uses stock `WebSocket.connect` without per-IP pinning (same v0.1 limitation as HTTPS).
